...
def main():
    ...
    with torch_tensorrt.logging.info():
        compiled_model = torch_tensorrt.compile(model, inputs=inputs)

    res2 = compiled_model(*inputs)
    ...
    max_diff = torch.max(torch.abs(res1.cpu() - res2.cpu())).item()

    print(f"Correct: {correct}, Max Diff: {max_diff}")
